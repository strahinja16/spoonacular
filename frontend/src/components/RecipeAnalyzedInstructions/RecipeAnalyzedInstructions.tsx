...
const RecipeAnalyzedInstructions: FC<RecipeDetailedPreparationProps> = ({ analyzedInstructions }) => {
  return (
    <section className="recipeDetailedPreparationSection">
      {analyzedInstructions.map(({ name, steps }) => {
        return (
          <section key={name}>
            <Row>{name}</Row>
            {steps.map(step => <AnalyzedInstructionStep key={step.number} step={step} />)}
          </section>
        );
      })}
    </section>
